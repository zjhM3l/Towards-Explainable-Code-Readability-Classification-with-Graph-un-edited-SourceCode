...
class GNNExplainer(ExplainerBase):
    ...
    def forward(self, data, mask_features=False, **kwargs):
        r"""
        ...
        """
        super().forward(x=data.x, edge_index=data.edge_index, **kwargs)
        self.model.eval()
        ...
        with torch.no_grad():
            related_preds = self.eval_related_pred(data, hard_edge_masks, node_idx=None)
