...
from explanation.utils.shapley import GnnNetsGC2valueFunc, GnnNetsNC2valueFunc, \
    gnn_score, mc_shapley, l_shapley, mc_l_shapley, NC_mc_l_shapley, sparsity
from dig.xgraph.evaluation.metrics import fidelity

"""
    module1: 可视化--plotutils
    module2: 核心运算--mcts，mctnode（借助shapley value来实现对mcts的探索）
    module3: 解释器--subgraphx
"""
...
class PlotUtils(object):
    ...
    def plot(self, graph, nodelist, figname, title_sentence=None, **kwargs):
        ...
        elif self.dataset_name.lower() in ['code_readability'.lower()]:
            x = kwargs.get('x')
            data_raw = kwargs.get('data_raw')
            edgelist = kwargs.get('edgelist')
            self.plot_code_readability(graph, nodelist, x, data_raw, edgelist=edgelist, figname=figname)
        else:
            raise NotImplementedError
    ...
    def plot_subgraph(self,
        ...
        nx.draw_networkx_edges(graph, pos, edgelist=edgelist, width=3, edge_color=edge_color, arrows=True, arrowstyle="->", arrowsize=10)

        nx.draw_networkx_edges(graph, pos=pos_nodelist,
                                   edgelist=edgeList, width=6,
                                   edge_color=critical_edge_colors,
    ...
    def plot_code_readability(self, graph, nodelist, x, data_raw, edgelist=None, figname=None):

        # element_idxs = {k: int(float(v) * 10) for k, v in enumerate(x[:, 0])}
        # node_idxs = element_idxs
        #
        # # 注意看：画图函数！！！
        # pos = nx.kamada_kawai_layout(graph)
        ...
        # loop_color = ['#4169E1', '#4169E1', '#4169E1', '#4169E1', '#4169E1', '#4169E1']
        # branch_color = ['#32CD32', '#32CD32', '#32CD32', '#32CD32', '#32CD32']
        # exception_color = ['#FFD700', '#FFD700', '#FFD700', '#FFD700', '#FFD700']
        # other_color = ['#A44100', '#A44100', '#A44100', '#A44100', '#A44100', '#A44100',
        #                '#A44100', '#A44100']
        # declaration_color = ['#890098', '#890098']
        ...
        edge_colors = ['#A2DEFF', '#A2FFB8', '#fb9de7']  # 蓝色，绿色，红色
        sub_edge_colors = ['#0588D4', '#00D231', '#ff2624']
        edge_color = [edge_colors[all_edge_types.index(v)] for v in edges_types]
        subgraph_edge_color = [sub_edge_colors[all_edge_types.index(v)] for v in edges_types]
        ...
        self.plot_subgraph(graph, nodelist, colors=colors, labels=node_labels,
                           ...
                           subgraph_edge_color=subgraph_edge_color,
...
class SubgraphX(object):
    ...
    """
    def __init__(self, model, num_classes: int, device, num_hops: Optional[int] = None, verbose: bool = True,
        ...
        self.reward_method = 'mc_l_shapley'
    ...
    def visualization(self, results: list, prediction, data_raw, edge_list, save_dir,
        ...
        if self.save:
            if vis_name is None:
                vis_name = os.path.join(save_dir, f"{self.filename}.png")
        else:
            vis_name = None
        tree_node_x = find_closest_node_result(results[prediction], max_nodes=max_nodes)
        if self.explain_graph:
    ...
    def read_from_MCTSInfo_list(self, MCTSInfo_list, prediction):
        ...
        # 我们的任务是MCTSInfo_list[0]是list
        elif isinstance(MCTSInfo_list[0], list):
            ret_list = MCTSInfo_list[prediction]
        return ret_list
    ...
    # 为了适应dmod模型，forward方法，参数改成data
    def explain(self, data, label: int,
                max_nodes: int = 5,
                node_idx: Optional[int] = None,
                saved_MCTSInfo_list: Optional[List[List]] = None):

        # model输出，predicted probability
        prob, _ = self.model(data)
        # prediction class：0，1，2
        prediction = prob.squeeze().argmax(-1).item()
        # MODIFY: 需要解释图
        self.explain_graph = True

        if self.explain_graph:
            if saved_MCTSInfo_list:
                results = self.read_from_MCTSInfo_list(saved_MCTSInfo_list, prediction)

            if not saved_MCTSInfo_list:
                # 构建mct探索的score function
                value_func = GnnNetsGC2valueFunc(self.model, target_class=label)
                payoff_func = self.get_reward_func(value_func)
                # 进行mct探索
                self.mcts_state_map = self.get_mcts_class(data.x, data.edge_index, score_func=payoff_func)
                # 解释result在此！！！
            ...
            self.mcts_state_map = self.get_mcts_class(data.x, data.edge_index, node_idx=node_idx)
        ...
        # 算fidelity
        fidelity_result = fidelity(prob.squeeze()[prediction], torch.tensor(maskout_score))
        related_pred = {'masked': masked_score,
                        'maskout': maskout_score,
                        'origin': prob.squeeze()[prediction].item(),
    ...
    def __call__(self, data, **kwargs)\
        ...
        for label_idx, label in enumerate(ex_labels):
            results, related_pred = self.explain(data,
                                                 label=label,
                                                 max_nodes=max_nodes,
                                                 node_idx=node_idx,
        ...
        if self.save:
            # 保存的是explanation_results是一个list，里面包含的是MCTSnode类的object
